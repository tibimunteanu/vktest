...
private:
    void createInstance()
    {
        //get and check required instance extensions
        std::vector<const char*> requiredExtensions = getRequiredInstanceExtensions();
        if (!checkRequiredInstanceExtensions(requiredExtensions))
        {
            throw std::runtime_error("Vulkan required instance extensions are not available!");
        }
        ...
        //structs used to create a vulkan instance
        ...
        //setup an additional debug messenger specifically for the vkCreateInstance and vkDestroyInstance calls
        ...
        if (enableValidationLayers)
        {
            ...
            configureDebugMessenger(debugCreateInfo);
    ...
    void setupDebugMessenger()
    {
        ...
        configureDebugMessenger(createInfo);
    ...
    void pickPhysicalDevice()
    {
        ...
        //check if the best device has required features (score > 0)
        ...
        {
            throw std::runtime_error("No GPUs with required Vulkan features found!");
        }
    }
    ...
    void createSwapChain()
    {
        ...
        //get the actual imageCount and swap chain images
    ...
    void createImageViews()
    {
        ...
        for (size_t i = 0; i < swapChainImages.size(); i++)
        {
            ...
            {
                throw std::runtime_error("Failed to create Vulkan swap chain image views!");
            }
        }
    }
    ...
    void createSyncObjects()
    {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
        {
            ...
            {
                throw std::runtime_error("Failed to create Vulkan synchronization objects for a frame!");
            }
        }
    }
...
private:
    ...
    bool checkRequiredInstanceExtensions(std::vector<const char*> requiredExtensions)
    {
    ...
    void configureDebugMessenger(VkDebugUtilsMessengerCreateInfoEXT& createInfo)
    {
        createInfo = {};
    ...
    uint32_t rateDevice(VkPhysicalDevice device)
    {
        ...
        //ensure required device extensions are pressent
        if (!checkRequiredDeviceExtensions(device))
        {
            return 0;
        }

        //ensure swap chain capabilities (at least one supported image format and one supported presentation mode for now)
